...
export const ConversationProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
   ...
   * Also deletes the audio file from Firebase Storage and chat history
   */
  const deleteConversation = useCallback(async (id: string) => {
    ...
    try {
      // Clear chat history subcollection first (must happen before deleting parent doc)
      ...
      const audioStoragePath = conversation?.audioStoragePath;
      if (audioStoragePath) {
        await storageService.deleteAudio(audioStoragePath);
        audioUrlCache.current.delete(audioStoragePath);
      }

      console.log('[ConversationContext] Deleted conversation, chat history, and audio:', id);
    } catch (err) {
